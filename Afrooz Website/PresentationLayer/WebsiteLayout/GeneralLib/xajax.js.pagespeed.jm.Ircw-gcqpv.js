...
return data;}}
this.loadingFunction=function(sFunction,aArgs,sRequestType){if(xajax.customLoadingFunction)
xajax.customLoadingFunction(sFunction,aArgs,sRequestType);};this.doneLoadingFunction=function(sFunction,aArgs,sRequestType){if(xajax.customDoneLoadingFunction)
xajax.customDoneLoadingFunction(sFunction,aArgs,sRequestType);};var loadingTimeout;this.customLoadingFunction;this.customDoneLoadingFunction;this.customTimeoutFunction;this.timeout=4000;this.timeoutFunction=function(xmlHttpRequest,sFunction,aArgs,sRequestType){document.body.style.cursor='default';if(xajax.customTimeoutFunction)
xajax.customTimeoutFunction(xmlHttpRequest,sFunction,aArgs,sRequestType);};this.abortIfCallInProgress=function(xmlHttpRequest,sFunction,aArgs,sRequestType){if(xmlHttpRequest)
if(xmlHttpRequest.readyState==1){xmlHttpRequest.abort();xajax.timeoutFunction(xmlHttpRequest,sFunction,aArgs,sRequestType);}}
this.call=function(sFunction,aArgs,sRequestType){var i,r,postData;if(document.body&&xajaxWaitCursor)
document.body.style.cursor='wait';if(xajaxStatusMessages==true)window.status='Sending Request...';clearTimeout(loadingTimeout);loadingTimeout=setTimeout(function(){xajax.loadingFunction(sFunction,aArgs,sRequestType);},0);if(xajaxDebug)this.DebugMessage("Starting xajax...");if(sRequestType==null){var xajaxRequestType=xajaxDefinedPost;}
...
return;try{if(!r.status)return;}catch(e){return;}
if(r.status==200){if(xajaxDebug)xajax.DebugMessage("Received:\n"+r.responseText);if(r.responseXML&&r.responseXML.documentElement)
...
xajax.doneLoadingFunction(sFunction,aArgs,sRequestType);delete r;r=null;}
if(xajaxDebug)this.DebugMessage("Calling "+sFunction+" uri="+uri+" (post:"+postData+")");r.send(postData);if(xajaxStatusMessages==true)window.status='Waiting for data...';setTimeout(function(){xajax.abortIfCallInProgress(r,sFunction,aArgs,sRequestType);},this.timeout);delete r;return true;}
...
this.processResponse=function(xml){clearTimeout(loadingTimeout);if(xajaxStatusMessages==true)window.status='Processing...';var tmpXajax=null;xml=xml.documentElement;if(xml==null)
